...
async function run(req: NextRequest) {
  ...
  const [{ id: logId }] = await sql`
    ...
    VALUES ('find-tender.service.gov.uk', 'running')
    RETURNING id
  `;

  try {
    const tenders = await fetchAllActiveFtsTenders({ maxPages: 10, pageSize: 100, delayMs: 200 });
    ...
    for (const t of tenders) {
      await sql`
        INSERT INTO tenders (
          ...
          ${t.issuer_name}, ${t.issuer_country ?? "GB"},
    ...
    await sql`
      ...
      WHERE id = ${logId}
    `;
    console.error("FTS sync error:", message);
